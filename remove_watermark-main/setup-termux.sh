...
pkg update -y
pkg install -y python python-pip binutils clang

echo "[*] Fake ldd untuk PyInstaller..."
echo '#!/bin/bash' > $PREFIX/bin/ldd
echo 'echo "ldd (fake): $*" >&2' >> $PREFIX/bin/ldd
...
rm -f $PREFIX/bin/ldd 2>/dev/null
